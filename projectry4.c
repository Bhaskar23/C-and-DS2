...
void display(NODE *head)    //display all accounts present
{
    ...
    while (cur!=NULL)
    {
        ...
        printf("\n");
        //cur=cur->link;
        fprintf(fp,"list of Accounts are:\n");
           fprintf(fp,"name           :%s\n",cur->name);
           fprintf(fp,"Account Type   :%s\n",cur->acc_type);
           fprintf(fp,"Account Number :%.lf\n",cur->acc_no);
           fprintf(fp,"Aadhar Number  :%.lf\n",cur->aadhar_no);
           fprintf(fp,"Account balance:%.2f\n",cur->balance);
           fprintf(fp,"Mobile Number  :%.lf\n",cur->mobile_no);
           fprintf(fp,"\n");
           cur=cur->link;
    }
}
NODE*
 check_accno(double key,NODE *head)    //checking of account number
{
    NODE *cur;
  if(head==NULL)
  {
...
NODE* transfer(double sender,double receiver,double amnt,NODE *head)//transfer of amount
{
    NODE *sen,*rec;
     if(head==NULL)
    {
    ...
    while(sen!=NULL)
    {
        if(sen->acc_no==sender)
        {
            if((sen->balance=sen->balance-amnt)<0)
            {
...
int main()
{
    ...
    do
    {
      ...
      printf("1.CREATE ACCOUNT\t2.DELETE ACCOUNT\n3.DELETE\t        4.UPDATE ACC DETAILS\n5.DISPLAY ALL ACCOUNTS\t6.CHECK ACCOUNT DETAILS\n7.TRANSFER\n");
      scanf("%d",&ch);
      switch(ch)
      {
          case 1:
         ...
         case 7:
                ...
                head=transfer(sender,receiver,amnt,head);
                break;
